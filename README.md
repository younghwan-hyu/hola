...
카메라가 켜져 있는 동안, 브라우저는 대화와 **별개로** 주기적으로 프레임을 보내 단발 판정을 받습니다. 지금 등록된 체크는 `presence`(사용자의 **얼굴**이 화면에 보이는가) 하나이고, 얼굴이 안 보이는 게 연속 2회 확인되면 아바타가 먼저 "어디 가셨어요?" 하고 말을 겁니다. 아무도 없는 경우뿐 아니라 **몸통만 걸치고 얼굴은 프레임 밖으로 벗어난 경우, 뒤통수만 보이는 경우도 자리를 비운 것으로 봅니다**. 반대로 얼굴이 나오기만 하면 다른 곳을 보고 있든 일부가 가려졌든 어둡든 자리에 있는 것으로 봅니다.
...
export function createPresenceCheck(): PerceptionCheck {
  return {
    name: "presence",
    intervalMs: 3000,
    consecutive: 2,        // 몇 회 연속 걸려야 실행할지 (1 = 즉시)
