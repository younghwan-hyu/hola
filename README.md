...
- **문서 열기**: 뷰어의 "PDF 열기" 버튼 → 파일 선택창에서 **임의의 로컬 PDF**를 엽니다(사전 목록·서버 업로드 없음). 페이지 이전/다음 버튼으로 넘기고, "다른 PDF"로 언제든 파일을 교체합니다. 렌더링은 웹 번들에 포함된 **pdf.js**(worker 포함)로 브라우저에서만 수행됩니다.
