...
- **배경**: 아바타가 서 있는 3D 배경. 기본은 외부 에셋 없이 코드로 만든 **대학 강의실**(화이트보드·프로젝터 스크린·전자교탁·긴 책상)이고, 설정에서 **없음**(어두운 그라데이션)으로 바꿀 수 있음 (아래 [배경](#배경))
- **판정(perception)**: 카메라가 켜져 있는 동안 브라우저가 주기적으로 저해상도 프레임을 보내 "얼굴이 보이는지", "딴 걸 보고 있는지", "표정이 어두운지" 같은 단발 판정만 받고, 조건이 걸리면 아바타가 **먼저 말을 겁니다**. 체크가 여러 개여도 말은 한 번만 겁니다. 카메라 없이도 **1분 넘게 응답이 없으면** 뭘 하고 있는지, 문제는 없는지 먼저 묻습니다(브라우저 타이머, 모델 호출 없음). 음성으로 말한 턴은 **목소리(속도·톤)**도 판정해, 평소보다 느리고 머뭇거리거나 낮고 단조로우면 그 턴의 답변 방식을 조절합니다(모델 호출 없음). 어떤 판정을 돌릴지는 웹의 **상황 인지 버튼**에서 항목별로 켜고 끕니다. 대화 파이프라인과 분리된 경량 경로 (아래 [판정 경로](#판정-경로-perception))
...
- `text`: string
- `audio`: file (`audio/webm;codecs=opus` 또는 `audio/ogg;codecs=opus`. `audio/wav`(16-bit PCM)도 받음 — 스크립트 테스트용)
...
- `voice`: JSON 문자열 (선택, 음성 턴 전용) — 브라우저가 녹음을 분석한 톤 특징 `{ durationSec, speechSec, rmsMean, f0MeanHz, f0StdHz, voicedRatio }` (`web/src/lib/voice.ts`). 형식이 틀리면 무시됩니다.
- `perception`: 쉼표 구분 체크 이름 (선택, 음성 턴 전용) — 켜 둔 음성 체크. 없으면 음성 판정을 하지 않습니다.
...
- `perception` `{ check, label, text, signal? }` — 음성 턴에 대한 음성 체크의 판정 (`text`는 표시용 문구, `signal`은 그 턴의 메시지 끝에 붙인 알림 문장. `stt` 다음에 옴)
...
- `GET /api/perception` — 브라우저가 돌릴 체크 목록. `[{ name, label, description, requires, trigger, relative?, frame? }]`
  - `label` / `description` — 웹의 상황 인지 모달에 표시되는 이름·설명.
  - `requires` — 브라우저가 갖춰야 할 **선택적 장치**의 목록(현재 `"camera"`뿐). 하나라도 없으면 그 체크는 켜져 있어도 돌지 않고 대기합니다. 음성 체크와 타이머 체크는 비어 있습니다 — 음성 입력은 이 앱의 기본 입력이고, 타이머는 장치가 필요 없어 굳이 표시하지 않습니다.
  - `trigger` — 구동 방식. 폴링 `{ kind: "poll", intervalMs, consecutive }`(`requires`에 `"camera"`가 있으면 모달에 **"카메라 폴링"** 뱃지 — 주기는 표시하지 않음) 또는 턴 구동 `{ kind: "turn", input: "voice" }`(**"말할 때마다"** 뱃지 — `POST /api/chat`의 음성 턴 안에서 실행되며 `POST /api/perception/:name`으로는 부를 수 없음, 400), 또는 타이머 구동 `{ kind: "idle", afterMs }`(**"시간 측정"** 뱃지 — 브라우저가 시간을 재다 넘기면 `POST /api/perception/:name`을 이미지 없이 호출하고, 서버는 문구만 돌려줌).
  - `relative` — 절대 기준이 아니라 **그 사용자의 평소(세션별 기준선) 대비**로 판정하는 체크에만 `true` (현재 `voice`). 모달에 회색 **"상대적 측정"** 뱃지로 표시됩니다.
  - `frame` — 카메라 체크가 보낼 프레임 스펙 `{ maxPx, quality }` (`requires`에 `"camera"`가 있을 때).
  - 프롬프트·트리거 문구는 서버에만 있습니다.
- `POST /api/perception/:name` — multipart `image` (≤2MB). 응답 `{ label, signal? }`. `signal`은 아바타가 말을 걸어야 하는 판정일 때만 실리는 상태 알림 문장. 없는 체크는 404, 프로바이더 오류는 502. 타이머 체크(`idle`)는 이미지 없이 호출하며 항상 `{ label: "idle", signal }`을 돌려줍니다.

## RAG (문서 검색)

웹 하단 입력 행의 **"문서 업로드"(파일) 버튼**(`⋯`로 입력 바를 펼친 뒤 [설정][아바타][제스처][파일] 중 [파일]. 접힌 기본 행 [문서][카메라][음성][상황 인지][⋯]에는 없음)으로 텍스트 파일을 올리면, 서버가 문장/문단 경계 기준으로 청킹하고 **로컬 임베딩 모델**로 벡터화해 **pgvector**에 저장합니다. 임베딩은 외부 API가 아니라 compose의 `embeddings`(Ollama) 컨테이너에서 직접 구동됩니다.
...
웹 하단 입력 행의 **카메라 버튼**([문서][카메라][음성][상황 인지][⋯] 순)으로 카메라를 켜면, 우측 상단에 라이브 프리뷰가 뜨고 이후 **모든 대화 전송(텍스트·음성)에 그 순간의 프레임이 함께 전송**됩니다. 끄기는 다시 카메라 버튼으로. AI에게 이미지는 `[카메라]` 라벨과 함께 "사용자 카메라에 지금 비친 실시간 장면"으로 전달되어, 대화와 관련 있으면 확인해 답하고 무관하면 무시하도록 프롬프트됩니다(`server/src/ai/system-prompt.ts`).
...
**기본으로 켜져 있습니다**: 처음 열면 화면이 분할되어 **왼쪽에 PDF 뷰어, 오른쪽에 아바타**가 배치됩니다(가로 PC 레이아웃 전용, 60/40). 하단 입력 행 맨 왼쪽의 **문서 버튼**을 누르면 아바타 전체 화면으로 전환되고, 다시 누르면 뷰어로 복귀 — 열어 둔 문서와 페이지는 유지됩니다.
...
카메라가 켜져 있는 동안, 브라우저는 대화와 **별개로** 주기적으로 프레임을 보내 단발 판정을 받습니다. 여기에 음성 턴마다 도는 목소리 체크와, 카메라 없이 브라우저 타이머만으로 도는 무응답 체크가 더해져 지금 등록된 체크는 다섯 개입니다.

| 체크 | 묻는 것 | 주기 / 연속 | 걸리면 |
|---|---|---|---|
| `presence` | 사용자의 **얼굴**이 화면에 보이는가 | 3초 / 1회 (즉시) | "어디 가셨어요?" |
| `attention` | 얼굴은 보이는데 **화면이 아닌 다른 곳**을 보고 있는가 | 4초 / 2회 연속 (약 8초) | "뭘 보고 계세요?" |
| `expression` | 얼굴 **표정**이 어둡거나 아리송한가 | 5초 / 2회 연속 (약 10초) | "잘 이해되지 않았어요?" |
| `voice` | 말한 **목소리**가 평소보다 느리고 머뭇거리는가 / 낮고 단조로운가 | 음성 턴마다 (폴링 아님) | 그 턴의 답변 방식만 조절 (별도 발화 없음) |
| `idle` | **1분 넘게** 아무 응답(전송·입력)이 없는가 | 브라우저 타이머 (5초마다 확인, 카메라 불필요) | "뭐 하고 계세요? 문제 없으세요?" |
...
- `attention`: 얼굴은 보이는데 **시선이 화면을 벗어나 있을 때** 걸립니다 — 고개를 옆·아래·뒤로 돌려 화면 밖의 것을 보거나, 손에 든 휴대폰·책 등을 들여다보는 경우. 모니터를 보느라 시선이 웹캠보다 조금 아래를 향하는 것은 정상(집중)으로 봅니다. 잠깐 고개를 돌린 것에 반응하지 않도록 **두 번 연속** 걸려야 말을 겁니다. 화면에 사람이 없으면(그건 `presence`의 몫) 무해한 라벨로 떨어집니다.
...
- `voice`: 카메라가 아니라 **방금 말한 음성**을 봅니다. 속도·멈춤은 STT의 단어 타임스탬프(`enableWordTimeOffsets`)와 전사문의 음절 수로, 톤(피치 변동·음량)은 브라우저가 녹음에서 계산해 보낸 특징(`voice` 필드)으로 잽니다 — **모델 호출은 없습니다**. 판정은 절대값이 아니라 **그 사용자의 평소(대화 세션별 baseline, 서버 메모리)** 대비이며 처음 두 음성 턴은 기준만 잡습니다. 평소보다 느리고(75% 미만) 멈춤이 많거나 시작이 늦으면 `hesitant`, 피치 변동이 평소의 60% 미만이고 음량도 80% 미만이면 `flat`(안전망으로 2음절/초 미만·멈춤 45% 초과는 무조건 `hesitant`). 4음절·1초 미만의 짧은 말은 판단하지 않습니다. 걸리면 **그 턴의 사용자 메시지 끝에** `(perception: 사용자의 말이 평소보다 느리고 머뭇거립니다)` 같은 알림을 붙여 보내고, 시스템 프롬프트 규칙에 따라 답변만 조절합니다(`hesitant` → 더 짧고 쉽게·하나씩·끝에 이해 확인, `flat` → 핵심만 짧게·컨디션 한마디). 별도로 말을 걸지 않으므로 무장(`armed`)과 무관하고, 화면의 사용자 말풍선에는 알림이 보이지 않습니다. 규칙·문구는 `server/src/perception/voice.ts`, 브라우저 쪽 분석은 `web/src/lib/voice.ts`.
- `idle`: 센서도 모델도 쓰지 않는 **타이머 체크**입니다. 브라우저가 아바타의 마지막 턴이 끝난 시점(또는 마지막으로 입력창에 타이핑한 시점)부터 시간을 재고, 아바타가 아직 말하는 중인 시간은 세지 않습니다. `afterMs`(1분)를 넘기면 `POST /api/perception/idle`을 이미지 없이 호출해 문구만 받아 와 다른 체크와 똑같이 히든 턴으로 주입하고, 아바타가 뭘 하고 있는지·문제는 없는지 가볍게 묻습니다. 카메라가 꺼져 있어도 돌고, 탭이 백그라운드면 확인을 미뤘다가 돌아왔을 때 이미 넘겨 있으면 그때 말을 겁니다. 한 번 말을 걸면 공통 무장(`armed`) 규칙대로 사용자가 실제로 응답할 때까지 다시 걸지 않습니다. 시간은 `server/src/perception/idle.ts`의 `IDLE_AFTER_MS`.
...
음성 턴의 `voice` 체크는 위 경로와 달리 폴링도 별도 발화도 없이 `POST /api/chat` 안에서 끝납니다: STT(단어 타임스탬프) → `voice.analyze()` → (걸리면) 사용자 메시지 끝에 알림 부착 → AI. 브라우저는 녹음을 보내기 전에 톤 특징만 계산해 함께 보냅니다.
...
- **항목별 on/off**: 하단 입력 행의 **상황 인지 버튼**([문서][카메라][음성][상황 인지][⋯]의 네 번째)을 누르면 서버에 등록된 체크가 스위치와 함께 나열됩니다(이름·설명은 서버가 보내는 `label`/`description`). 각 항목에는 구동 방식 뱃지가 하나 붙습니다 — **"카메라 폴링"**(`requires`에 `"camera"` + `trigger.kind === "poll"`), **"말할 때마다"**(`trigger.kind === "turn"`), **"시간 측정"**(`trigger.kind === "idle"`). 구동 방식 뱃지는 파란색이고, `relative`인 체크에는 회색 **"상대적 측정"** 뱃지가 하나 더 붙습니다. 켜져 있지만 요구 조건이 빠진 체크(카메라가 꺼져 있는 카메라 체크)는 뱃지가 주황색으로 표시됩니다 — 카메라는 메인 행의 카메라 버튼으로 켭니다. 그 밖의 동작 상태(동작 중, 말을 걸고 대기 중 등)는 모달에 표시하지 않습니다. 체크는 **켜져 있고 요구 조건이 모두 충족될 때만** 폴링합니다 — 끈 체크는 폴링 타이머 자체가 돌지 않아 vision 호출이 나가지 않고, 판정 응답을 기다리는 도중에 끄면 그 결과도 버립니다. 선택은 브라우저에 저장되어(localStorage `hola.perceptionDisabled`, `web/src/lib/settings.ts`) 다음 방문에도 유지되며, 서버에 새로 추가된 체크는 기본 **켜짐**입니다. 버튼 자체는 카메라·문서 버튼과 달리 동작 여부에 따라 색이 바뀌지 않습니다. 켜고 끄는 것은 이미 걸어 둔 말(무장 상태)에는 영향을 주지 않습니다.

### 체크 추가하기

`server/src/perception/<name>.ts`에 팩토리 하나를 쓰고 `createPerceptionChecks()`에 등록하면 끝입니다. 라우트도, 브라우저 폴링 루프도 이 인터페이스에 대해 제네릭이라 **클라이언트 수정이 필요 없습니다**(상황 인지 모달의 on/off 목록에도 `label`/`description`으로 자동 등장). 음성 체크(`trigger.kind === "turn"`)는 `prompt`/`labels` 대신 `analyze(input, sessionKey)`를 구현합니다 — `server/src/perception/voice.ts` 참고. 타이머 체크(`trigger.kind === "idle"`)는 `signal` 문구 하나만 갖습니다 — `server/src/perception/idle.ts` 참고. 발화가 겹칠 걱정도 없지만(위 "말은 한 번만"), 어느 체크가 이기든 그 프레임에 맞는 말이 나오도록 **라벨은 서로 배타적으로** 설계하세요 — 예를 들어 `attention`은 "화면에 사람이 없음"을 자기 트리거로 삼지 않고 무해한 라벨로 넘깁니다(그건 `presence`가 볼 상황).
...
export function createPresenceCheck(): PerceptionCheck {
  return {
    ...
    description: "얼굴이 카메라에 비치는지 확인합니다.", // 모달의 한 줄 설명 — 무엇을 보는지만, 반응 방식은 쓰지 않음
    requires: ["camera"],  // 브라우저가 갖춰야 할 것 — 없으면 켜져 있어도 대기
    trigger: {             // 구동 방식 → camera + poll 이면 "카메라 폴링" 뱃지
      kind: "poll",
      intervalMs: 3000,
...
> ⚠️ **비용**: 카메라를 켜 두면 등록된 체크들이 각자의 주기로 vision 호출을 냅니다 (지금 설정으로 분당 최대 47회 — `presence` 20 + `attention` 15 + `expression` 12. `voice`·`idle`은 모델을 호출하지 않아 비용이 없습니다). 아바타가 이미 말을 걸어 둔 상태에서는 호출이 나가지 않으므로, 자리를 비운 채 방치할 때는 0회입니다. 주기를 늘리려면 `trigger.intervalMs`를, 체크를 빼려면 `createPerceptionChecks()`에서 제거하세요. `AI_MODEL`은 카메라 기능과 마찬가지로 vision 지원 모델이어야 합니다.
...
- **인라인**: 3D 화면 **위에** 최근 말풍선 2개만 겹쳐 뜨고, 새 말풍선이 생기면 가장 오래된 것이 위로 밀려 사라집니다. 입력 바도 3D 위에 떠 있습니다.
- **분리** (기본): 3D 화면 **아래**에 대화 로그 창이 생기고, 그 아래로 입력 바가 내려갑니다. 지나간 말풍선이 사라지지 않고 쌓이므로 **스크롤해서 다시 볼 수 있습니다**. 새 메시지가 오면 자동으로 맨 아래를 따라가되, 위로 올려 읽는 중이면 끌려가지 않습니다. 3D 영역이 그만큼 낮아지므로 **카메라가 얼굴 쪽으로 더 다가간 기본 구도**로 잡힙니다(`SEPARATE_CAMERA_DISTANCE`, 기본 1.5m 대 인라인 2.2m — 마우스 휠로 언제든 당기고 밀 수 있습니다).
...
아바타 뒤의 3D 배경은 입력 바를 펼친 뒤(`⋯`) **설정(톱니) 버튼**의 **배경**에서 고릅니다. 선택은 브라우저에 저장되어(localStorage `hola.background`, `web/src/lib/settings.ts`) 다음 방문에도 유지됩니다.

- **강의실** (기본): `web/src/lib/classroom.ts`가 외부 에셋 없이 three.js 기본 도형과 캔버스 텍스처만으로 만드는 한국 대학 강의실 — 앞쪽 벽 전체의 화이트보드(빈 상태, 마커·지우개만 받침에)와 그 가운데로 내려온 프로젝터 스크린(강의 목차 슬라이드 투사 — 텍스트만, 아바타 머리가 가리는 중앙은 비워 두 단으로 배치), 시계·스피커, 모니터와 마이크가 달린 전자교탁, 4명씩 앉는 긴 연결 책상과 파란 의자 4열, 회색 타일 바닥, 격자 천장의 형광등 패널과 프로젝터, 블라인드가 내려온 창문, 문·게시판·벽걸이 에어컨, 아바타 발밑 그림자. 아바타는 원점에서 +z(카메라 쪽)를 보고 서므로 보드는 -z, 책상은 +z에 두고, 카메라가 줌아웃하는 가운데 통로는 비워 둡니다. 보드 벽은 아바타 뒤 4m로 멀리 두어, 문서 조회 모드의 좁은 화면(기본 구도에서 그 깊이의 벽이 약 2m 폭만 보임)에서도 스크린이 거의 통째로 들어오고 배경이 멀리 있는 것처럼 보입니다. 조명은 아바타용 기존 조명을 그대로 쓰되, 스크린·형광등·천장처럼 스스로 빛나거나 조명을 등진 면은 조명을 받지 않는 재질입니다.
- **없음**: 배경 없이 어두운 그라데이션 위에 아바타만.

전환은 씬을 다시 만들지 않고 배경 그룹만 교체하므로 아바타가 다시 로드되지 않습니다(`Avatar.tsx`의 `background` prop). 배경을 추가하려면 `web/src/lib/backgrounds.ts`의 `createBackground()`에 팩토리를, `settings.ts`의 `BACKGROUNDS`에 항목을 더하면 됩니다 — `Avatar.tsx`와 설정 모달은 목록에 대해 제네릭입니다.

## 로딩 화면

아바타(VRM)를 불러오는 동안은 **화면 전체를 덮는 로딩 화면**만 보입니다 — 스피너와 "아바타 불러오는 중…", 그리고 파일 크기를 알 수 있을 때는 다운로드 진행률(%)과 진행 막대. 아래에서는 앱 전체(문서 뷰어, 3D 뷰, 입력 바)가 처음부터 마운트되어 배치와 VRM 로드를 진행하고, 아바타가 **첫 프레임을 그린 순간** 커버가 0.5초 페이드아웃되며 모든 UI가 한 번에 드러납니다(`App.tsx`의 `splash` 상태, 진행률은 `Avatar`의 `onProgress` → `lib/vrm.ts`의 `GLTFLoader` 진행 이벤트). 아바타를 바꿀 때도 같은 커버가 뜨고, 로드에 실패하면 커버는 걷히고 3D 영역에 오류 메시지가 표시됩니다 — 아바타 없이도 대화는 동작하므로 앱을 막지 않습니다.
