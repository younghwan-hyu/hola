...
- `GET /api/perception` — 브라우저가 돌릴 체크 목록. `[{ name, label, description, requires, trigger, frame? }]`
  ...
  - `requires` — 브라우저가 갖춰야 할 **선택적 장치**의 목록(현재 `"camera"`뿐). 모달에 **"카메라 필요"** 뱃지로 표시되고, 하나라도 없으면 그 체크는 켜져 있어도 돌지 않고 대기합니다. 음성 체크는 비어 있습니다 — 음성 입력은 이 앱의 기본 입력이라 굳이 표시하지 않습니다.
  - `trigger` — 구동 방식. 폴링 `{ kind: "poll", intervalMs, consecutive }`(모달에 **"N초 폴링"** 뱃지) 또는 턴 구동 `{ kind: "turn", input: "voice" }`(**"말할 때마다"** 뱃지 — `POST /api/chat`의 음성 턴 안에서 실행되며 `POST /api/perception/:name`으로는 부를 수 없음, 400).
