...
- `GET /api/perception` — 브라우저가 돌릴 체크 목록. `[{ name, label, description, requires, trigger, relative?, frame? }]`
  ...
  - `requires` — 브라우저가 갖춰야 할 **선택적 장치**의 목록(현재 `"camera"`뿐). 하나라도 없으면 그 체크는 켜져 있어도 돌지 않고 대기합니다. 음성 체크는 비어 있습니다 — 음성 입력은 이 앱의 기본 입력이라 굳이 표시하지 않습니다.
  - `trigger` — 구동 방식. 폴링 `{ kind: "poll", intervalMs, consecutive }`(`requires`에 `"camera"`가 있으면 모달에 **"카메라 폴링"** 뱃지 — 주기는 표시하지 않음) 또는 턴 구동 `{ kind: "turn", input: "voice" }`(**"말할 때마다"** 뱃지 — `POST /api/chat`의 음성 턴 안에서 실행되며 `POST /api/perception/:name`으로는 부를 수 없음, 400).
...
카메라가 켜져 있는 동안, 브라우저는 대화와 **별개로** 주기적으로 프레임을 보내 단발 판정을 받습니다. 지금 등록된 체크는 네 개입니다.
...
> ⚠️ **비용**: 카메라를 켜 두면 등록된 체크들이 각자의 주기로 vision 호출을 냅니다 (지금 설정으로 분당 최대 47회 — `presence` 20 + `attention` 15 + `expression` 12. `voice`는 모델을 호출하지 않아 비용이 없습니다). 아바타가 이미 말을 걸어 둔 상태에서는 호출이 나가지 않으므로, 자리를 비운 채 방치할 때는 0회입니다. 주기를 늘리려면 `trigger.intervalMs`를, 체크를 빼려면 `createPerceptionChecks()`에서 제거하세요. `AI_MODEL`은 카메라 기능과 마찬가지로 vision 지원 모델이어야 합니다.
